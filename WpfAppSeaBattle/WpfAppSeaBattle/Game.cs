...
namespace WpfSeaBattle
{
    public class Game
    {
        ...
        void PlaceRandomShips(CellType[,] field) 
        {
            ...
            while (true)
            {
                ...
                if (isHorizontal)
                {
                    ...
                    cnt = CountFirstNotVisitedDownCells(y, x, visited); //J: swapped x and y
                    TransposeMatrix(visited);
                }
                else
                    cnt = CountFirstNotVisitedDownCells(x, y, visited); //J: swapped x and y

                //int right = x + cnt - 1;//J: what for?
                if (cnt >= currentSize)
                {
                    if (isHorizontal) 
                    {
                        ...
                        TransposeMatrix(visited);
                        PlaceShip(y, x, currentSize, field, visited);//J: swapped x and y, changed cnt to currSize
                        TransposeMatrix(field);
                        TransposeMatrix(visited);
                    }
                    else
                       PlaceShip(x, y, currentSize, field, visited); //J: swapped x and y, changed cnt to currSize
        ...
        void PlaceShip(int x, int y, int length, CellType [,] field, bool [,] visited)
        {
            ...
            {
                field[i, y] = CellType.Ship; //J places down
            }
            for (int i = x - 1; i <= x + length; i++)
            {
                for (int j = y - 1; j <= y + 1; j++) //Ok
                {
                    if (IsInField(i, j)) //J: deleted && field[i, j] == CellType.Empty (or add visited[i, y] = true; in for)
                        visited[i, j] = true;
                }
            }
        }
        ...
        {
            int cnt = 1;
            int down = x + 1; //J: now - down, was - right
        ...
        bool shipIsDestroyed(int x, int y, CellType[,] field) //bad
        {
            //right
        ...
        void CheckFourSides(int x, int y, CellType[,] field) //this sucks; need to find out if the ship is horisontal or not
        {
        ...
        public int CheckSmbdWonLost() //wtf it's awful
        {
